...
SC_MODULE(myinstructionspliter) {
    sc_in<myword> instruction, instruction2;
    ...
    void m() {
        ...
        myword instruction2_ = instruction2.read();

        for (int i{0}; i < MYWORD_LENGTH;i ++) {

            bool i_bit = instruction_.get_bit(i);
            bool i_bit2 = instruction2_.get_bit(i);
            
            if (i <= 15) {
                D.set_bit(i, i_bit);
                if (i <= 5) {E.set_bit(i, i_bit);}
                if (i >= 11) {C.set_bit(i - 11, i_bit2);}
            }
            else {
                if (i <= 20) {B.set_bit(i - 16, i_bit2);}
