...
SC_MODULE(myregisterbank) {
    ...
    void m() {
        if (write.read() == 1) { // clk.read() == 0
            ...
            std::cout << "REG WRITE!!! " << my_to_int<myaddressword>(addr_write.read()) % banksize << " " << data.read() << '\n';
        } else {

            //std::cout << "REG READ!!! "<< bank[ my_to_int<myaddressword>(addr1.read()) % banksize ] << '\n';
